...
**This is not a throwaway PoC — if results stay favourable, CRDT is the way forward.** So the "accept-degraded" rows are TEMPORARY waypoints, not destinations. Two of them are hard **PROD-BLOCKERS** that MUST be built before CRDT carries production traffic: **(i) SASL X3 relay** (X3-based SASL is non-negotiable) and **(ii) bouncer over the mesh** (enabled mesh-wide today; BX cross-mesh is a live correctness break). Degradation is acceptable *for now* only because the bed masks them (local-Keycloak for SASL; no cross-mesh alias traffic exercised yet for bouncer). Track them as blockers, not follow-ons.
...
- **MR-5-5c — CH accept-degraded hardening.** Skip anchored `server_ads[]` entries so a federated `CH Q` doesn't wedge a `FedRequest` on an impossible reply (clean timeout). No CR-M build. (Genuine accept-degraded: every node is STORE so federation is redundancy, not correctness.)
