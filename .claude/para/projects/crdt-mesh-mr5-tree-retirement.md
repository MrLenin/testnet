...
> Status: **SCOPED 2026-06-17; MR-5-0 (shadow oracle) DONE + GREEN-BOARD-validated (submodule `45540d1`).** Source-grounded (Plan-agent pass). Flag: SEPARATE `FEAT_CRDT_TREE_RETIRE` (see §6 — diverged from the agent's reuse-the-shared-flag advice).
...
- **MR-5-2 (suppress SERVER relay — `m_server.c:959`).** Turn the MR-5-0 `(void)crdt_server_intro_suppress(bcptr, acptr)` into `if (crdt_server_intro_suppress(bcptr, acptr)) continue;`; flip `FEAT_CRDT_TREE_RETIRE`. The relayed-third-server path (replacement most-proven). **Exit = the R7b regression oracle (§7).**
- **MR-5-3 (suppress at `server_estab` — `s_serv.c:206,287`).** Same discard→`continue` conversion at both sites. Cold-link view; **MR-5-0 CONFIRMED the cold-link gap is real (`present=0` at estab)** ⇒ pair with the event-driven beacon-burst at `server_estab` (re-evaluate the R7b-reverted experiment).
