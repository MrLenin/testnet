...
> Status: **THE KEYSTONE IS DONE + LIVE-VALIDATED (R7b OVERTURNED). MR-5-0 (`45540d1`) + MR-5-1 (`e48083c`) + MR-5-2/5-3 SERVER suppression (`d1fe32a`) all landed.** Flag SEPARATE `FEAT_CRDT_TREE_RETIRE` (§6). Only remaining MR-5 refinement = the event-driven beacon-burst (cold-link bringup latency) + MR-5-5 (stateful-subsystem decision MD/CH/MR/BS-BX/CI).
