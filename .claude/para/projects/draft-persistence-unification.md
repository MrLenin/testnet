...
| **M4b** ✅ | Network-level PART suppression via the bouncer reconciler.  Held-account /PART on profile A is suppressed when the union of profiles still wants the channel; affected aliases lose CHFL_ALIAS via new BX V subcommand, parting primary/alias gets synthetic PART echo.  Cascade path runs when union empty.  /JOIN-already-member case emits synthetic JOIN + single-channel state burst via BX V +.  `bounce_sync_alias_join` filtered by per-alias effective profile.  HOLD-sticky collapses to "non-held = not a bouncer session" (account-level hold gates the whole reconciler). | suppress when other profile wants; cascade when union empty; non-held bypass |
