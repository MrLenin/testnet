...
```
When a session has two connections on different servers and one of them sends a PM, the *other* client should also see it so the conversation is complete on every device. BX E is the wire encoding of that cross-server delivery. Since 2026-09-15 it is only emitted toward, and only delivered to, a member that negotiated `echo-message`: the echo is a message sourced from the user's own nick, and a client without echo-message files it under its source, i.e. a query window with the user's own nick (BX M has the same gate on delivery). TARGET_NUMERIC is the session member that should receive the echo, FROM_NUMERIC is the sender's alias, TOK is the IRC verb (PRIVMSG/NOTICE), PM_TARGET is the original recipient nick, MSGID is the IRCv3 msgid to ensure dedup.
...
| FIELD=VALUE | String | Identity-update key/value (BX U). `host`/`realhost`/`realname`/`fakehost`/`cloakhost`/`cloakip`/`username`/`account` (identity mirroring), `caps=<hex>` (BX_CAP_* bitmask: 0x01 draft/multiline, 0x02 batch, 0x04 echo-message (2026-09-15; lets the sending server skip a session echo to a member that cannot place it)), `profile=<name>` (active persistence profile), `la=<unix ts>` (activity, 2026-09-03: emitted by a connection's own server on its first message after a quiet period of min(5 min, half the account's webpush idle window) (2026-09-11, `bounce_activity_quiet`; was a flat 5 min); the numeric may name an ALIAS or the PRIMARY — receivers update `ba_last_active` or `hs_last_active` and forward; it is handled before the alias guard so a primary's numeric is never deferred). Consumers: promotion (most recently active alias wins), session-wide WHOIS idle, oper CHECK per-connection idle, webpush attention. `aw=<0\|1\|2>` (2026-09-11: the connection's OWN away state, 0 present / 1 away / 2 `AWAY *` per draft/pre-away; emitted by its server on every change in the session path and in the link burst for locally hosted aliases that are not present; receivers keep `ba_away`/`hs_primary_away` + `_known` and forward). Consumers: webpush attention for remote members (instead of the session's mirrored effective away, which a present sibling hides), away aggregation (a remote `*` is invisible, like a local one) |
